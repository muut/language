moot.language = {

   // the language code
   $code: 'pl',

   /* New stuff */
   ban:     'Ban user',
   banned:  'Banned',
   ban_usr: 'Banned users',
   ban_acc: 'Banned account',
   ban_msg: 'You are temporarily banned from posting on this forum.',
   days:    'Days',
   forever: 'Forever',
   slogan:  'Free forums and commenting by Muut',

   add: '+ Add',
   addons: 'Add-ons',
   myaccount: "My account",
   untitled: "Untitled",
   design: "Design",
   no_title: "Musisz podać tytuł",
   designer: "Designer",
   developer: "Developer",

   no_index: 'Disable indexing under muut.com',
   mkpriv: 'Make this forum private',
   sign: 'This forum must be <a>signed</a> first.',

   s3: 'Custom S3 bucket',
   s3_tip: 'Allows custom domain for search engines and a backup storage.',
   s3_tip2: 'Add full access permissions for <em>moot-aws@moot.it</em>',
   s3_no_access: "moot-aws@moot.it cannot access this bucket",
   s3_missing: "This bucket does not exist",
   img_needed: "Please upload an image file",


   max: 'Maksymalny rozmiar zdjęcia to 4MB',
   yes: 'Tak',
   upload: 'Załącz zdjęcia',

   show: 'pokaż',
   hide: 'ukryj',
   invisible: 'Niewidoczny',
   givebody: 'Treść nie może być pusta.',
   show_old: 'Pokaż najstarsze odpowiedzi',
   show_more: 'Pokaż więcej odpowiedzi',
   show_new: 'Pokaż najnowsze odpowiedzi',
   start: 'Załóż nowy temat...',
   single: 'pojedyncze słowo',
   bold: 'pogrubienie',
   italics: 'pochylenie',
   monospaced: 'stała szerokość czcionki',
   domain: 'URL forum',
   domaintip: 'Pełne URL do forum, gdzie użytkownicy są przekierowywani z wyników wyszukiwania',
   learn: 'Dowiedz się więcej...',
   ga: 'ID w Google Analytics',
   admins: 'Administratorzy',
   adminstip: 'Jedna nazwa użytkownika na linię',
   settings: 'Ustawienia',
   error: 'Ups, napotkaliśmy błąd. Prosimy o skopiowanie postu do schowka, przeładowanie strony i spróbowania wysłania jeszcze raz. Przepraszamy - naprawimy to!',
   body: 'Tekst treści',
   admin: 'Administrator',
   someone: 'Ktoś',
   writing: 'pisze...',
   basic: 'Podstawowe (za darmo)',
   subs: 'Subskrypcje',
   manage: 'Zarządzaj',
   fullview: 'Pełen widok',
   title: 'Tytul',
   link: 'link',
   avatar: 'Zmień awatar',
   seo_update: 'Strony SEO będą zaktualizowane w przeciągu jednej godziny',
   upgrade_link: 'Ulepsz',
   branding: 'Marka',
   logo: 'Logo',
   bgcolor: 'Kolor tła',
   brand_name: 'Nazwa marki',
   css: 'Plik CSS',
   mootbrand: 'Użyj omarkowania Muut',
   sso: 'Pojedyncze logowanie',
   apikey: 'Klucz API',
   apisecret: 'Sekretny klucz',



   /* End new stuff */


   username_fail: 'Tylko małe litery, cyfry lub myślnik (\'-\')',
   all: 'Wszystkie', // or alternatively: All

   anon_1: 'anonimowy',    // singular
   anon: 'anonimowi',      // plural

   badforum: 'Nieprawidłowe forum',
   c_acc1: 'Możesz teraz korzystać ze swojego konta na wszystkich stronach Muut',
   c_acc: 'Konto zweryfikowane',
   c_email: 'Adres email zweryfikowany',
   cancel: 'Anuluj',
   change: 'Zmień',
   changepw: 'Zmień hasło',
   close: 'Zamknij',
   collapse: 'Zwiń wszystkie',
   comm_count_1: 'komentarz',   // singular: "1 comment"
   comm_count_n: 'komentarze',  // plural: "3 comments"
   comment1: 'Dodaj pierwszy komentarz...',
   comment: 'Komentarz...',
   edit: 'edycja',
   email: 'Email',
   email_taken: 'Ten email jest zajęty',
   emailtip: 'Twój email nie zostanie udostępniony',
   enter: 'ctrl + enter',
   expired1: 'Właśnie wysłaliśmy Ci nowy. Sprawdź swój email.', // http://d.pr/i/1OEV
   expired: 'Ten link wygasł ',

   feed: 'Moje wątki', // alternatives: My own, personal

   forumname: 'Nazwa forum',
   forums: 'Fora',

   help2: 'post',
   help3: 'tekst cytowany',
   help5: 'kod, język zgadnięty', //??
   help6: 'kod HTML',
   in_forum: 'w',
   invalid: 'Nieprawidłowa wartość',
   join: 'Dołącz do Muut',
   later: 'Później...',
   less: 'mniej',
   like: 'Lubię',
   likepost: 'Like ten post',
   loading: 'Ładowanie...',
   login: 'Zaloguj',
   login1: 'Na początku się zaloguj', //??
   login_and: "Zaloguj i",

   logout: 'Wyloguj',
   mark: 'Obrażający albo spam',
   mean: 'Czy miałeś na myśli',
   name: 'Wyświetlane imię',
   nametip: 'Twoje prawdziwe imię, dzięki temu zostaniesz rozpoznany przez ludzi',
   newcat: 'Nowa kategoria',
   newpass: 'Nopwe hasło',
   noresults: 'Nie ma wyników dla',
   nospam: 'Zero spamu na Twoim forum!',
   notexist: 'nie istnieje.',
   notify: 'Informuj mnie o nowych wydaniach',
   ok: 'ok',
   online: 'Online',
   post: 'Post',
   preview: 'Podgląd',
   progress: ['ładowanie...', 'dalej ładuje...', 'i ładuje...', 'rezygnuje'],
   reconnect: 'Ponowne łączenie...',
   removecat: 'Usuń. Posty nie zostaną usunięte',
   removepost: 'Usuń',
   reply: 'Odpowiedz...',
   replybtn: 'Odpowiedz',
   save: 'Zapisz zmiany',
   saved: 'zapisano',
   search: 'Szukaj',
   select: 'Wybierz forum',
   setup: 'Przejdź do ustawień', //??
   showing: 'Pokazuje',
   spam: 'Spam',
   step1: 'Wysłaliśmy email z potwierdzeniem. Twój email nie zostanie zmienio dopóki nie ukończysz tego kroku!', //??
   step: 'Jeszcze jeden krok',
   terms: 'Zgadzam się na <a>warunki używania</a>.',
   thanks: 'Dzięki!',
   uname_len: 'Wpisz 6 lub więcej znaków',
   uname_taken: 'Ta nazwa użytkownika jest zajęta!',
   unametip: 'Nie będziesz mógł tego później zmienić',
   undo: 'Cofnij',
   unlike: 'Przestań lubić',
   unspam: 'Nie spam',
   unver1: 'Niezweryfikowane konto',
   unver2: 'Sprawdź email aby zweryfikować konto. Zweryfikowani użytkownicy mogą śledzić wątki i otrzymywać emaile. <a>Wyślij nowy kod weryfikacyjny...</a>',
   unver3: '<a>Wyślij nowy</a> kod weryfikacyjny na:',
   unver4: "Wysłano. Sprawdź email.",
   unver: 'Niezweryfikowany',
   unwall: 'przestań śledzić wszystkie',
   unwatch: 'Przestań śledzić',
   upgrade2: 'Muut używa WebSockets albo CORS do komunikacji między domenami. Wszystkie nowoczesne przeglądarki oprócz Opery 11 i IE7 będą działać.', //?
   upgrade: 'Proszę zainstalować nową wersję przeglądarki',
   username_taken: 'Ta nazwa użytkownika jest zajęta',
   watch: 'Śledź nowe odpowiedzi',
   welcome1: 'Sprawdź email aby dokończyć rejestrację. Jeśli nie otrzymałeś emaila w przeciągu kilku minut sprawdź folder spam.',

   welcome2: 'Proszę podać nazwę użytkownika Muut',
   welcome: 'Witaj w Muut',
   you: 'Ty',

   // times
   now: 'przed chwilą',
   months: "Sty,Lut,Mar,Kwi,Maj,Cze,Lip,Sie,Wrz,Paź,Lis,Gru",
   min: 'm',
   hour: 'h',
   day: 'd',
   week: 't'

}
